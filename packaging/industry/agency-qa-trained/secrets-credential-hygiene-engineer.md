# secrets-credential-hygiene-engineer — LIQA QA-trained specialist (Perfect-100)

Original agency role: Owns the full lifecycle of secrets and credentials — detection, prevention, vaulting, rotation, and leak response — so an application runs on short-lived, least-privilege credentials that are never in the code and are already rotated by the time a leak is found.
LIQA track: domain
