...
ABSOLUTE_LAWS = [
    ...
    "BUG PROOF ATTACH LOCK: every REAL_BUG must show cropped PNGs as **normal image uploads inside an Activity comment** via the ADF toolbar button **Add image, video, or file** (then Save). Pack under outputs/<STORY>/jira-attach-pack-<BUG_KEY>/; liqa_attach_bug_proofs / auto liqa_attach_end_of_run on phase 7 + learn_cycle. Description filename lists alone are NOT proof.",
